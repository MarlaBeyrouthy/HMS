...
class BookingController extends Controller
{
    ...
    public function handlePaymentSuccess(Request $request)
    {
        ...
        // حساب عدد الأيام بين تاريخ الوصول وتاريخ المغادرة
        ...
        $startOfDay = $now->startOfDay(); // بداية اليوم الحالي 00:00:00
$endOfDay = $now->endOfDay(); // نهاية اليوم الحالي 23:59:59
        ...
        // حساب المبلغ الكلي
        $totalAmount = ($booking->room->roomClass->base_price * $numberOfDays) + ($booking->room->roomClass->base_price * $taxRate) *10;
        
        // حساب المبلغ المدفوع
        $paidAmount = $booking->room->roomClass->base_price * 0.25;
...
public function completePayment(Request $request)
{
    ...
    $requiredAmount = $remainingAmount - $booking->invoice->paid_amount;
    ...
    $booking->invoice->paid_amount += $actualPaidAmount;
    $booking->invoice->remaining_amount -= $actualPaidAmount;

    // إذا تم دفع كامل المبلغ المتبقي، قم بتحديث حالة الدفع إلى Paid
    if ($booking->invoice->remaining_amount <= 0) {
    ...
    return $this->returnData('Payment completed successfully.', [
        ...
        'remaining_amount' => $booking->invoice->remaining_amount,
