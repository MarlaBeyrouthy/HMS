...
class RoomController extends Controller
{
    ...
    public function searchRooms(Request $request)
    {
        ...
        // البحث عن الغرف بناءً على معايير البحث المحددة
        $rooms = Room::query();
    
        foreach ($searchCriteria as $key => $value) {
            // تحقق مما إذا كانت المعايير موجودة وقم بإضافتها إلى استعلام البحث
            if ($value) {
                if ($key === 'status') {
                    // إذا كانت المعايير تتعلق بحالة الغرفة، استخدم عملية التصفية المناسبة
                    $rooms->where('status', $value);
                } else if ($key === 'room_number') {
                    // إذا كانت المعايير تتعلق برقم الغرفة، استخدم عملية التصفية المناسبة
                    $rooms->where('room_number', $value);
                } else {
                    // لأي معايير أخرى، قم بتطبيق عملية البحث الجزئي
                    $rooms->where($key, 'like', '%' . $value . '%');
                }
            }
        }
        ...
        if ($results->isEmpty()) {
            return $this->returnErrorMessage('No rooms found with the provided criteria.', 'E003');
        }
        
        return $this->returnData('Rooms retrieved successfully', $results);
    }
