...
public function updateBooking(Request $request)
{
    ...
    // Check if the selected room is available for the new dates
    $existingBooking = Booking::where('room_id', $request->room_id)
        ->where(function ($query) use ($request) {
            $query->where('check_in_date', '<=', $request->check_out_date)
                ->where('check_out_date', '>=', $request->check_in_date);
    ...
    if (!$invoice) {
        return $this->returnErrorMessage('Invoice not found for booking.', 'E009', 404);
    }
    ...
    if ($newRemainingAmount > 0) {
        ...
        try {
            ...
            // Retrieve original payment intent associated with the booking
            $paymentIntent = \Stripe\PaymentIntent::retrieve($booking->payment_session_id);

            if (!$paymentIntent) {
                return $this->returnErrorMessage('Original payment intent not found.', 'E008', 404);
            }

            // Create a refund for the excess amount
