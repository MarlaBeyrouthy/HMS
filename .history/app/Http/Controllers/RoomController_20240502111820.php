...
class RoomController extends Controller
{
    ...
    public function getAllRooms()
    {
        ...
        foreach ($rooms as $room) {
            $bookings = $room->bookings()->where('check_out_date', '>', now())->get();
    
            // التحقق مما إذا كان هناك حجوزات مستقبلية
            if ($bookings->isEmpty()) {
                $room->availability_message = 'Available with no bookings.';
            } else {
                $room->availability_message = 'Booked.';
            }
