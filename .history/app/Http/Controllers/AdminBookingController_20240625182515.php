...
class AdminBookingController extends Controller
{
    ...
    public function showDetails($id)
    {
        $user = Auth::user();
        if (!$user) {
            return $this->returnErrorMessage('User not authenticated.', 'E401', 401);
        }

        if ($user->permission_id !== 2) {
            return $this->returnErrorMessage('You do not have permission to perform this action.', 'E403', 403);
        }
