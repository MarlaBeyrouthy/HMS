...
// تابع لمعالجة الدفع بنجاح وتحديث الفاتورة
public function handlePaymentSuccess(Request $request)
{
    // التحقق من وجود معرف الجلسة
    $this->validate($request, [
    ...
    $booking->payment_status = '// تابع لمعالجة الدفع بنجاح وتحديث الفاتورة
    public function handlePaymentSuccess(Request $request)
    {
        // التحقق من وجود معرف الجلسة
        $this->validate($request, [
        ...
        // جلب اسم المستخدم ورقم الغرفة وإرجاعها مع الفاتورة
        $username = $booking->user->first_name;
        $roomNumber = $booking->room->room_number;
        return $this->returnData('Payment successful.', [
            'username' => $username,
