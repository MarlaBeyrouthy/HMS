...
class BookingController extends Controller
{
    ...
    public function handlePaymentSuccess(Request $request)
    {
        ...
        // حساب عدد الأيام بين تاريخ الحجز وتاريخ الإلغاء
        $numberOfDays = $startDate->diffInDays($endDate);
    
        // حساب المبلغ المتبقي بعد دفع العربون
        $basePrice = $booking->room->roomClass->base_price;
        $remainingAmount = $basePrice - ($basePrice * 0.25 * $numberOfDays);
    
        // إنشاء أو تحديث الفاتورة
        $invoice = Invoice::updateOrCreate(
            ['booking_id' => $booking->id],
            [
                'total_amount' => $basePrice * $numberOfDays + ($basePrice * $taxRate * $numberOfDays),
                'paid_amount' => $basePrice * 0.25 * $numberOfDays,
                'remaining_amount' => $remainingAmount,
                'taxes' => $basePrice * $taxRate * $numberOfDays,
                'invoice_date' => now(),
            ]
        );
