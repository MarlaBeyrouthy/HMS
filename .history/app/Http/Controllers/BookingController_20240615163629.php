...
public function uBooking(Request $request)
{
    // التحقق من صحة البيانات المدخلة
    $validator = Validator::make($request->all(), [
        'booking_id' => 'required|exists:bookings,id',
    ...
    // التحقق من توافر الغرفة الجديدة في التواريخ المطلوبة
    $existingBooking = Booking::where('room_id', $request->room_id)
        ->where(function ($query) use ($request) {
            $query->where('check_in_date', '<=', $request->check_out_date)
                  ->where('check_out_date', '>=', $request->check_in_date);
    ...
    // حساب المبلغ الإجمالي الجديد (المبلغ الأساسي + مبلغ الضريبة)
    $newTotalAmount = $newBaseAmount + $newTaxes;

    // حساب المبلغ المتبقي بناءً على المبلغ المدفوع مسبقاً
    $paidAmount = $booking->invoices->paid_amount;
    $newRemainingAmount = $newTotalAmount - $paidAmount;

    // تحديث الفاتورة
    $booking->invoices->base_amount = $newBaseAmount;
    $booking->invoices->total_amount = $newTotalAmount;
    $booking->invoices->remaining_amount = $newRemainingAmount;
    $booking->invoices->taxes = $newTaxes;
    $booking->invoices->invoice_date = now();
    $booking->invoices->save();

    // التعامل مع الفروقات في الدفع
    if ($newRemainingAmount > 0) {
        // المبلغ الجديد المطلوب دفعه أكبر من المبلغ المدفوع مسبقاً
        // إنشاء جلسة Stripe جديدة للدفع
        try {
            Stripe::setApiKey("sk_test_51NedBNEQbJiqtI6xmfmqk6fHT6g1DmnNWTncMeoGQLVYZn8e86HvEHBQk390lhS6fEYL4DDzTjT1sCKJhV2tZpN000krSm6HlX");
                ...
                'line_items' => [
                    [
                        'price_data' => [
                            ...
                            'unit_amount' => $newRemainingAmount * 100, // المبلغ المتبقي
                        ],
                        'quantity' => 1,
                    ],
                ],
        ...
        // المبلغ المدفوع مسبقاً أكبر من المبلغ الجديد المطلوب دفعه
        // استرداد المبلغ الزائد عبر Stripe
        try {
            Stripe::setApiKey("sk_test_51NedBNEQbJiqtI6xmfmqk6fHT6g1DmnNWTncMeoGQLVYZn8e86HvEHBQk390lhS6fEYL4DDzTjT1sCKJhV2tZpN000krSm6HlX");

            $refund = \Stripe\Refund::create([
                'amount' => abs($newRemainingAmount) * 100, // المبلغ الزائد
                'payment_intent' => $booking->payment_session_id,
            ]);

            // تحديث الفاتورة والمبلغ المدفوع
            $booking->invoices->paid_amount = $newTotalAmount;
            $booking->invoices->remaining_amount = 0;
            $booking->invoices->save();
