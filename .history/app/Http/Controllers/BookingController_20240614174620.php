...
لجعل دالة تعديل الحجز (updateBooking) تتناسب مع الكود الحالي، يجب الأخذ بعين الاعتبار عملية دفع الفروقات عبر Stripe إذا كانت طريقة الدفع المختارة هي Stripe. سيتم تعديل الدالة للتأكد من إجراء الدفع بشكل صحيح عند تعديل الحجز، سواء بوجود مبلغ إضافي يجب دفعه أو مبلغ يجب استرداده.

إليك الكود المعدل لدالة `updateBooking`:
...
public function updateBooking(Request $request)
{
    ...
    // حساب عدد الأيام السابقة وعدد الأيام الجديدة
    $oldCheckInDate = new \DateTime($booking->check_in_date);
    $oldCheckOutDate = new \DateTime($booking->check_out_date);
    $oldNumberOfNights = $oldCheckInDate->diff($oldCheckOutDate)->days;

    $newCheckInDate = new \DateTime($request->check_in_date);
    $newCheckOutDate = new \DateTime($request->check_out_date);
    $newNumberOfNights = $newCheckInDate->diff($newCheckOutDate)->days;

    $roomClass = $booking->room->roomClass;
    $basePricePerNight = $roomClass->base_price;

    // حساب المبلغ الإجمالي الجديد
    $newTotalAmount = $basePricePerNight * $newNumberOfNights;

    // حساب الفرق في المبلغ
    $oldTotalAmount = $basePricePerNight * $oldNumberOfNights;
    $differenceAmount = $newTotalAmount - $oldTotalAmount;
    ...
    if ($booking->invoices) {
        $paidAmount = $booking->invoices->paid_amount; // المبلغ المدفوع سابقاً
        $remainingAmount = $newTotalAmount - $paidAmount; // المبلغ المتبقي

        // حساب الضرائب
        $taxRate = 0.1;
        $taxes = $newTotalAmount * $taxRate;

        // تحديث تفاصيل الفاتورة
        $booking->invoices->total_amount = $newTotalAmount;
        $booking->invoices->remaining_amount = $remainingAmount;
        $booking->invoices->taxes = $taxes;
        $booking->invoices->invoice_date = now();
        $booking->invoices->save();
    }

    // إذا كان هناك مبلغ إضافي يجب دفعه
    if ($differenceAmount > 0) {
        if ($request->payment_method == 'stripe') {
            try {
                ...
                // إنشاء جلسة دفع جديدة في Stripe
                $sessionId = Str::uuid()->toString();
                $session = Session::create([
                    'payment_method_types' => ['card'],
                    'line_items' => [
                        [
                            'price_data' => [
                                ...
                                'unit_amount' => $differenceAmount * 100, // المبلغ الإضافي
                            ],
                            'quantity' => 1,
                        ],
                    ],
                    'mode' => 'payment',
                    'success_url' => route('booking.success') . '?booking_id=' . $booking->id . '&session_id=' . $sessionId,
                    'cancel_url' => route('booking.cancel'),
                ]);
            ...
            return $this->returnErrorMessage('Additional payment required. Please use Stripe for payment.', 'E009', 400);
        }
    } elseif ($differenceAmount < 0) {
        // إذا كان هناك مبلغ يجب استرداده
        $refundAmount = abs($differenceAmount);

        if ($request->payment_method == 'stripe') {
            try {
                // إجراء استرداد المبلغ عبر Stripe
                $refund = \Stripe\Refund::create([
                    'amount' => $refundAmount * 100,
                    'payment_intent' => $booking->payment_session_id,
                ]);

                // تحديث المبلغ المسترد في الفاتورة
                $booking->invoices->paid_amount -= $refundAmount;
                $booking->invoices->remaining_amount += $refundAmount;
                $booking->invoices->save();

                return $this->returnSuccessMessage('Booking updated successfully. Refund issued: ' . $refundAmount);
            } catch (\Exception $e) {
                return $this->returnErrorMessage($e->getMessage(), 'E003', 500);
            }
        } else {
            ...
            return $this->returnSuccessMessage('Booking updated successfully. Refund amount: ' . $refundAmount);
        }
    } else {
        return $this->returnSuccessMessage('Booking updated successfully. No change in amount.');
    }
}
```

### التغييرات التي تم إجراؤها:
1. تم إضافة تحقق لوجود فروقات في المبلغ عند تعديل الحجز (إضافة أو استرداد).
2. إذا كانت هناك فروقات في المبلغ، يتم إنشاء جلسة دفع جديدة عبر Stripe لدفع المبلغ الإضافي.
3. إذا كان هناك مبلغ يجب استرداده، يتم إصدار استرداد المبلغ عبر Stripe إذا كانت طريقة الدفع هي Stripe.
4. تم تضمين التحقق من صحة المدخلات المطلوبة (طريقة الدفع) لضمان أن العملية تتم بشكل صحيح بناءً على طلب المستخدم.

بذلك، يكون لديك الآن دالة لتعديل الحجز تتناسب مع جميع السيناريوهات الممكنة عند تعديل الحجز، بما في ذلك دفع أو استرداد المبالغ باستخدام Stripe.
