...
class AdminBookingController extends Controller
{
    ...
    public function updatePaymentStatus(Request $request, $id)
    {
        ...
        $booking->save();
        $invoiceData = $this->bookingService->updateInvoiceBasedOnPaymentStatus($booking);
        return $this->returnData('Payment status updated successfully', [$booking, 'invoice' => $invoiceData]);
    }

    public function createBooking(Request $request)
    {
        // Validate the request
        $validator = $this->bookingService->validateBookingForCustmoerRequest($request);
        if ($validator->fails()) {
            return $this->returnErrorMessage($validator->errors()->first(), 'E001', 400);
        }
    ...
    public function createBookingForCustomer(Request $request)
    {
        // Validate the request
        $validator = $this->bookingService->validateBookingForCustmoerRequestvalidateBookingRequest($request);
        if ($validator->fails()) {
            return $this->returnErrorMessage($validator->errors()->first(), 'E001', 400);
        }
        // Check for existing bookings
