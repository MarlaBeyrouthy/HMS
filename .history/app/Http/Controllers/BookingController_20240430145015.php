...
class BookingController extends Controller
{
   ...
   public function completePayment(Request $request)
   {
       ...
       // حساب المبلغ الكلي للحجز بما في ذلك الضرائب
       $totalAmount = $booking->room->roomClass->base_price + ($booking->room->roomClass->base_price * $taxRate);
       
       // تحديث الفاتورة بالمبلغ الكلي
       $invoice = $booking->invoice;
       $invoice->total_amount = $booking->room->roomClass->base_price + ($booking->room->roomClass->base_price * $taxRate);;
       $invoice->save();
   
       // جلب اسم المستخدم ورقم الغرفة وإرجاعها
       $username = $booking->user->first_name;
       $roomNumber = $booking->room->room_number;
       return $this->returnData('Payment completed.', [
           'username' => $username,
           'room_number' => $roomNumber,
           'invoice' => $invoice
       ]);
