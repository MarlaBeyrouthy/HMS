...
public function updateBooking(Request $request)
    {
        ...
        // معالجة الفرق في المبلغ باستخدام Stripe
        if ($differenceAmount > 0) {
            // يجب دفع مبلغ إضافي
            try {
                $charge = Charge::create([
                    'amount' => $differenceAmount * 100, // تحويل المبلغ إلى سنت
                    'currency' => 'usd',
                    'customer' => $booking->stripe_customer_id, // معرف العميل في Stripe
                    'description' => 'Additional payment for booking ID: ' . $booking->id,
                ]);
                return $this->returnSuccessMessage('Booking updated successfully. Additional amount paid: ' . $differenceAmount);
            } catch (\Exception $e) {
                return $this->returnErrorMessage('Error in processing payment: ' . $e->getMessage(), 'E009', 400);
            }
        } elseif ($differenceAmount < 0) {
            // يجب استرداد مبلغ
            try {
                $refund = Refund::create([
                    'charge' => $booking->stripe_charge_id, // معرف الشحن في Stripe
                    'amount' => abs($differenceAmount) * 100, // تحويل المبلغ إلى سنت
                ]);
                return $this->returnSuccessMessage('Booking updated successfully. Refund amount: ' . abs($differenceAmount));
            } catch (\Exception $e) {
                return $this->returnErrorMessage('Error in processing refund: ' . $e->getMessage(), 'E010', 400);
            }
        } else {
