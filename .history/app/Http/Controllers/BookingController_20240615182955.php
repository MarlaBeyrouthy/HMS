...
public function updateBooking(Request $request)
{
    ...
    // تحديث الفاتورة
    $invoice = $booking->invoices;
    $lastRemainigAmount= $invoice->remaining_amount;
    $invoice->total_amount = $totalAmount;
    $invoice->remaining_amount = $remainingAmount;
    $invoice->taxes = $totalAmount * $taxRate;
    $invoice->save();

    // التحقق مما إذا كان هناك مبالغ مستحقة أو استرداد
    if ($remainingAmount > $lastRemainigAmount) {
        // يجب على المستخدم دفع المبلغ المتبقي
        return $this->returnData('Booking updated successfully. Additional payment required.', [
            'remaining_amount' => $remainingAmount,
            'invoice' => $invoice,
        ], 200);
    } elseif ($remainingAmount < $lastRemainigAmount) {
        // يجب استرداد المبلغ الفائض للمستخدم
        return $this->returnData('Booking updated successfully. Refund required.', [
            'refund_amount' => abs($remainingAmount),
            'invoice' => $invoice,
        ], 200);
    } else {
        // لا يوجد مبلغ إضافي أو استرداد
