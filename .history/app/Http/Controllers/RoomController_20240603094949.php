...
class RoomController extends Controller
{
    ...
    public function searchRooms(Request $request)
    {
        ...
        $rooms = $query->with(['roomClass', 'bookings'])->get();
        Room
        // Update room status based on current bookings
        foreach ($rooms as $room) {
            $currentBookings = $room->bookings->where('check_out_date', '>', now());
            ...
            if ($currentBookings->isEmpty()) {
                ...
                foreach ($currentBookings as $booking) {
                    ...
                    $occupied[] = [
                        'start' => $booking->check_in_date->toDateTimeString(),
                        'end' => $booking->check_out_date->toDateTimeString()
                    ];
                }
