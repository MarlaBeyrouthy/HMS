...
public function updateBooking(Request $request)
    {
        ...
        $booking = Booking::with('room', 'user', 'invoices')->find($request->id);
        if (!$booking) {
            return $this->returnErrorMessage('Booking not found.', 'E006', 404);
        }
        ...
        if ($booking->user_id !== Auth::id()) {
            return $this->returnErrorMessage('Unauthorized access.', 'E010', 403);
        }

        // تحديث حالة الغرفة القديمة إلى متاحة إذا تغيرت الغرفة
        ...
        // البحث عن الغرفة الجديدة وتحديث حالة الغرفة الجديدة إلى "محجوزة"
        $room = Room::findOrFail($request->room_id);
        if ($room->status == 'maintenance') {
            return $this->returnErrorMessage('The room is currently unavailable for booking.', 'E002', 400);
        }
        ...
        // معالجة الدفع إذا كانت طريقة الدفع هي Stripe
        if ($request->payment_method == 'stripe') {
            try {
                Stripe::setApiKey("sk_test_51NedBNEQbJiqtI6xmfmqk6fHT6g1DmnNWTncMeoGQLVYZn8e86HvEHBQk390lhS6fEYL4DDzTjT1sCKJhV2tZpN000krSm6HlX");
