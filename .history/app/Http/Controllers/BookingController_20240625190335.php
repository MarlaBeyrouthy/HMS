    ...
    public function makeBooking(Request $request)
{
    ...
    if (Auth::user()->permission_id != 2) {
        // If user is not admin, they can only create bookings for themselves
    ...
    // Check if user is banned from making bookings
    $user = User::find($request->input('user_id'));
    if ($user->permission_id == 4) {
        return $this->returnErrorMessage('The user is banned from making bookings.', 'E004', 403);
    }
    ...
    // If existing booking is found and payment status is 'cancel', delete it
    if ($existingBooking && $existingBooking->payment_status == 'cancel') {
        $existingBooking->delete();
    }

    // If existing booking is found and payment status is not 'cancel', return error
    if ($existingBooking && $existingBooking->payment_status != 'cancel') {
    ...
    try {
        ...
        return $this->returnData('Booking created successfully.', [
            'payment_method' => $request->payment_method,
            'session_id' => isset($sessionId) ? $sessionId : null,
            'booking_id' => $booking->id,
            'invoice' => $invoiceData,
        ], 200);
    } catch (\Exception $e) {
