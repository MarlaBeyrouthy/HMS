...
class RoomController extends Controller
{
    ...
    {
        $room = Room::with(['roomClass', 'bookings', 'wishlistedBy', 'reviews'])->find($id);

        // التحقق مما إذا تم العثور على الغرفة
        if (!$room) {
        ...
        // إرجاع تفاصيل الغرفة مع أوقات الانشغال
        return response()->json(['room' => $roomDetails]);
    }
    }
    ...
    public function getRoomDetails($id)
    {
        ...
        if (!$room) {
            return $this->returnErrorMessage('Room not found', 'E001', Response::HTTP_NOT_FOUND);

        }
        return $this->returnData('Room Details :', $room);
